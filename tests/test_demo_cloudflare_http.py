...
def _server() -> Generator[DemoServer, None, None]:
...
@pytest.mark.usefixtures("_server")
def test_healthz_wrong_path_returns_404() -> None:
...
@pytest.mark.usefixtures("_server")
def test_missing_hmac_headers_returns_401() -> None:
...
@pytest.mark.usefixtures("_server")
def test_bad_hmac_signature_returns_401() -> None:
    body = json.dumps({"action": "demo.read_status"}).encode()
...
@pytest.mark.usefixtures("_server")
def test_allow_path_returns_executed_true() -> None:
    body = json.dumps(
        {
...
@pytest.mark.usefixtures("_server")
def test_block_path_returns_executed_false() -> None:
    body = json.dumps(
        {
